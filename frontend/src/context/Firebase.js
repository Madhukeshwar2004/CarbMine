...
export const FirebaseProvider = (props) => {
    ...
    const addUser = async (CoalName, email, password) => {
        try {
            ...
            const userDoc = { CoalName, email, userId: loggedInUser.uid };
    ...
    const uploadPDFToFirebase = async (pdfBlob) => {
        try {
            ...
            await addDoc(collection(firestore, "users", user.uid, "pdfs"), { url: downloadURL, createdAt: new Date(), userId: user.uid });
    ...
    const fetchUserPDFs = async () => {
        try {
            ...
            return pdfsSnapshot.docs.map(doc => doc.data());
        } catch (error) {
